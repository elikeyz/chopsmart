...
def main():
  print("ChopSmart API - Docker Deployment")
  print("=========================================")
  ...
  terraform_dir = Path(__file__).parent.parent / "terraform" / "backend"
  ...
  docker_run_dir = Path(__file__).parent.parent
  os.chdir(docker_run_dir)
  ...
  print(
    "\nNext step: Run 'terraform apply' in terraform/backend to create the App Runner service."
  )
  ...
  try:
    services = run_command(
      [
        ...
        region,
        "--query",
        "ServiceSummaryList[?ServiceName=='chopsmart-api'].ServiceArn",
        "--output",
        "json",
      ],
      capture_output=True,
    ...
    if services:
      ...
      print("  2. Select 'chopsmart-api' service")
